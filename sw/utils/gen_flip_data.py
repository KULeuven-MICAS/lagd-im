...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate sw/include/model_f_data.h from the cluster files of one core."
    )
    parser.add_argument(
        "--core-onload",
        type=int,
        default=0,
        help="Index of the core this data is loaded onto, selecting clusters_<2n+1>/<2n+2> "
             "and the flip data section (default: 0).",
    )
    parser.add_argument(
        "--folder",
        type=str,
        default="default",
        help="Folder name under sw/tests/ref/data/ containing the cluster files",
    )
    parser.add_argument(
        "--suffix",
        type=str,
...
# Core n is loaded with clusters_(2n+1) and clusters_(2n+2): core 0 -> 1/2, core 1 -> 3/4.
INPUT_NAME_1 = f"clusters_{2 * core_onload + 1}"
...
with open(OUTPUT_FILE, 'w') as f:
    ...
    f.write(f"// interleaved from {INPUT_NAME_1} (even indices)"
            f" and {INPUT_NAME_2} (odd indices).\n")
...
print(f"  {INPUT_NAME_1}: {NUM_CLUSTERS} vectors")
print(f"  {INPUT_NAME_2}: {NUM_CLUSTERS} vectors")
