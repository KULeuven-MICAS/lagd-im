...
// writes stop working consistently across the array. See galena_sweep_wwl_read_timing.spm.c
// for the read-side counterpart -- kept as a separate test since there is no reason to expect the
...
//
// To isolate a write failure from a read failure, every verification read in this test runs at
// WWL_CYCLES_SAFE (deliberately generous, not just LAGD_BASE_CONFIG's default -- see below), while
// only the write side is swept down. That way a mismatch can only be attributed to the write.
//
