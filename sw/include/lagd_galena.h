...
static void galena_stage_spin_wwl(unsigned core, int active) {
    ...
    for (unsigned i = 0; i < LAGD_SPIN_NUM_WORDS; i++) pattern[i] = active ? 0xFFFFFFFFu : 0x00000000u;
    lagd_core_write_vector(core, LAGD_CORE_SPIN_WWL_STROBE_0_REG_OFFSET, pattern, LAGD_SPIN_NUM_WORDS);
}
...
static void galena_stage_spin_feedback(unsigned core, int active) {
    uint32_t pattern[LAGD_SPIN_NUM_WORDS];
    for (unsigned i = 0; i < LAGD_SPIN_NUM_WORDS; i++) pattern[i] = active ? 0xFFFFFFFFu : 0x00000000u;
    lagd_core_write_vector(core, LAGD_CORE_SPIN_FEEDBACK_CFG_0_REG_OFFSET, pattern, LAGD_SPIN_NUM_WORDS);
}
...
static void galena_stage_wwl_timing(unsigned core, unsigned cycle_per_wwl_high, unsigned cycle_per_wwl_low) {
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_1_REG_OFFSET, LAGD_CORE_COUNTER_CFG_1_CYCLE_PER_WWL_HIGH_MASK, LAGD_CORE_COUNTER_CFG_1_CYCLE_PER_WWL_HIGH_OFFSET, cycle_per_wwl_high);
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_2_REG_OFFSET, LAGD_CORE_COUNTER_CFG_2_CYCLE_PER_WWL_LOW_MASK, LAGD_CORE_COUNTER_CFG_2_CYCLE_PER_WWL_LOW_OFFSET, cycle_per_wwl_low);
}

// Load the WBL-read synchronizer's pipe depth
static void galena_stage_wbl_sync_depth(unsigned core, unsigned pipe_num) {
    lagd_core_write_field(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_SYNCHRONIZER_WBL_PIPE_NUM_MASK, LAGD_CORE_GLOBAL_CFG_1_SYNCHRONIZER_WBL_PIPE_NUM_OFFSET, pipe_num);
}

// Load the bct_read_o synchronizer's pipe depth.
static void galena_stage_spin_sync_depth(unsigned core, unsigned pipe_num) {
    lagd_core_write_field(core, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_2_SYNCHRONIZER_PIPE_NUM_MASK, LAGD_CORE_GLOBAL_CFG_2_SYNCHRONIZER_PIPE_NUM_OFFSET, pipe_num);
}

// Load the spin write pulse width.
static void galena_stage_spin_write_timing(unsigned core, unsigned cycle_per_spin_write) {
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_2_REG_OFFSET, LAGD_CORE_COUNTER_CFG_2_CYCLE_PER_SPIN_WRITE_MASK, LAGD_CORE_COUNTER_CFG_2_CYCLE_PER_SPIN_WRITE_OFFSET, cycle_per_spin_write);
}
...
static void galena_stage_feedback_pulse_timing(unsigned core, unsigned cycle_per_spin_compute) {
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_3_REG_OFFSET, LAGD_CORE_COUNTER_CFG_3_CYCLE_PER_SPIN_COMPUTE_MASK, LAGD_CORE_COUNTER_CFG_3_CYCLE_PER_SPIN_COMPUTE_OFFSET, cycle_per_spin_compute);
}

// Load the sequenced spin readout's per-sample cycle count and sample count.
static void galena_stage_spin_read_timing(unsigned core, unsigned cycle_per_spin_read, unsigned read_num) {
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_3_REG_OFFSET, LAGD_CORE_COUNTER_CFG_3_DEBUG_CYCLE_PER_SPIN_READ_MASK, LAGD_CORE_COUNTER_CFG_3_DEBUG_CYCLE_PER_SPIN_READ_OFFSET, cycle_per_spin_read);
    lagd_core_write_field(core, LAGD_CORE_COUNTER_CFG_4_REG_OFFSET, LAGD_CORE_COUNTER_CFG_4_DEBUG_SPIN_READ_NUM_MASK, LAGD_CORE_COUNTER_CFG_4_DEBUG_SPIN_READ_NUM_OFFSET, read_num);
}
...
static void galena_commit_spin_configure(unsigned core) {
    lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_CONFIGURE_ENABLE_BIT);
...
static void galena_commit_aw_configure(unsigned core) {
    lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_2_CONFIG_VALID_AW_BIT);
...
//     -captures into debug_wbl_read_data/_wblb; poll the sticky idle bit, not the missable valid pulse
...
static void galena_trigger_spin_write(unsigned core) {
    lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_WRITE_EN_BIT);
    lagd_core_clear_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_WRITE_EN_BIT);
}
...
static void galena_trigger_spin_read(unsigned core) {
    lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_READ_EN_BIT);
    lagd_core_clear_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_READ_EN_BIT);
}
...
//            result, and bct_read_o reverts to the originally written spin the instant feedback deasserts.
//            To actually observe a convergence, use galena_trigger_spin_read with feedback staged on instead.
static void galena_trigger_feedback_pulse(unsigned core) {
    lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_COMPUTE_EN_BIT);
    lagd_core_clear_bit(core, LAGD_CORE_GLOBAL_CFG_1_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_COMPUTE_EN_BIT);
}
...
static void galena_poll_spin_write_idle(unsigned core) {
    lagd_core_poll_bit(core, LAGD_CORE_OUTPUT_STATUS_REG_OFFSET, LAGD_CORE_OUTPUT_STATUS_DEBUG_SPIN_W_IDLE_BIT, 1);
}

static void galena_poll_feedback_pulse_idle(unsigned core) {
    lagd_core_poll_bit(core, LAGD_CORE_OUTPUT_STATUS_REG_OFFSET, LAGD_CORE_OUTPUT_STATUS_DEBUG_SPIN_CMPT_IDLE_BIT, 1);
}

static void galena_poll_spin_read_idle(unsigned core) {
    lagd_core_poll_bit(core, LAGD_CORE_OUTPUT_STATUS_REG_OFFSET, LAGD_CORE_OUTPUT_STATUS_DEBUG_SPIN_R_IDLE_BIT, 1);
}
...
static void galena_enable_spin_out(unsigned core, int enable) {
    if (enable) lagd_core_set_bit(core, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_2_CTNUS_DGT_DEBUG_BIT);
    else        lagd_core_clear_bit(core, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET, LAGD_CORE_GLOBAL_CFG_2_CTNUS_DGT_DEBUG_BIT);
}
...
static void galena_read_spin_flip_mem(unsigned core, unsigned index, uint32_t out[LAGD_SPIN_NUM_WORDS]) {
    volatile uint32_t *base = (volatile uint32_t *)(IC_J_MEM_END_ADDR +
        (uintptr_t)core * (L1_J_MEM_SIZE_B + L1_FLIP_MEM_SIZE_B));
    for (unsigned i = 0; i < LAGD_SPIN_NUM_WORDS; i++) out[i] = base[index * LAGD_SPIN_NUM_WORDS + i];
}
