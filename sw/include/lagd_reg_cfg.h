...
static void lagd_configure_counters(unsigned core) {
    ...
    *reg32(base, LAGD_CORE_COUNTER_CFG_2_REG_OFFSET) = cfg2;
    ...
    uint32_t cfg3 = (
        ...
        ((DEBUG_CYCLE_PER_SPIN_READ & LAGD_CORE_COUNTER_CFG_3_DEBUG_CYCLE_PER_SPIN_READ_MASK) << 16)
    );
    *reg32(base, LAGD_CORE_COUNTER_CFG_3_REG_OFFSET) = cfg3;
    ...
    uint32_t cfg4 = (
        ...
        ((ICON_LAST_RADDR_PLUS_ONE & LAGD_CORE_COUNTER_CFG_4_ICON_LAST_RADDR_PLUS_ONE_MASK) << 16)
    );
...
static void lagd_configure_global_cfg_1(unsigned core) {
    ...
    uint32_t cfg1 = (
        ...
        ((GCFG1_DEBUG_DT_CONFIGURE_ENABLE   & 0x1                                                   ) << LAGD_CORE_GLOBAL_CFG_1_DEBUG_DT_CONFIGURE_ENABLE_BIT   ) |
        ((GCFG1_DEBUG_SPIN_CONFIGURE_ENABLE & 0x1                                                   ) << LAGD_CORE_GLOBAL_CFG_1_DEBUG_SPIN_CONFIGURE_ENABLE_BIT ) |
        ((GCFG1_EN_PERF_COUNTER             & 0x1                                                   ) << LAGD_CORE_GLOBAL_CFG_1_EN_PERF_COUNTER_BIT             ) |
        ((GCFG1_BYPASS_DATA_CONVERSION      & 0x1                                                   ) << LAGD_CORE_GLOBAL_CFG_1_BYPASS_DATA_CONVERSION_BIT      ) |
        ...
        ((GCFG1_CONFIG_COUNTER              & LAGD_CORE_GLOBAL_CFG_1_CONFIG_COUNTER_MASK            ) << LAGD_CORE_GLOBAL_CFG_1_CONFIG_COUNTER_OFFSET           ) |
        ...
        ((GCFG1_SYNCHRONIZER_WBL_PIPE_NUM   & LAGD_CORE_GLOBAL_CFG_1_SYNCHRONIZER_WBL_PIPE_NUM_MASK ) << LAGD_CORE_GLOBAL_CFG_1_SYNCHRONIZER_WBL_PIPE_NUM_OFFSET)
    );
...
static void lagd_configure_global_cfg_2(unsigned core) {
    ...
    uint32_t cfg2 = (
        ...
        ((GCFG2_SYNCHRONIZER_PIPE_NUM       & LAGD_CORE_GLOBAL_CFG_2_SYNCHRONIZER_PIPE_NUM_MASK ) << LAGD_CORE_GLOBAL_CFG_2_SYNCHRONIZER_PIPE_NUM_OFFSET   ) |
        ((GCFG2_DEBUG_H_WWL                 & 0x1                                               ) << LAGD_CORE_GLOBAL_CFG_2_DEBUG_H_WWL_BIT                ) |
        ((GCFG2_DGT_ADDR_UPPER_BOUND        & LAGD_CORE_GLOBAL_CFG_2_DGT_ADDR_UPPER_BOUND_MASK  ) << LAGD_CORE_GLOBAL_CFG_2_DGT_ADDR_UPPER_BOUND_OFFSET    ) |
        ...
        ((GCFG2_CONFIG_SPIN_INITIAL_SKIP_0  & 0x1                                               ) << LAGD_CORE_GLOBAL_CFG_2_CONFIG_SPIN_INITIAL_SKIP_0_BIT ) |
        ((GCFG2_CONFIG_SPIN_INITIAL_SKIP_1  & 0x1                                               ) << LAGD_CORE_GLOBAL_CFG_2_CONFIG_SPIN_INITIAL_SKIP_1_BIT ) |
        ((GCFG2_DGT_HSCALING                & LAGD_CORE_GLOBAL_CFG_2_DGT_HSCALING_MASK          ) << LAGD_CORE_GLOBAL_CFG_2_DGT_HSCALING_OFFSET            )
    );
...
// and: DEBUG_DT_CONFIGURE_ENABLE, DEBUG_SPIN_CONFIGURE_ENABLE in global_cfg_1 register (if they are set).
static void lagd_clear_config_valid(unsigned core) {
    void *base = (void *)((uintptr_t)IC_REGS_BASE_ADDR +
                          (uintptr_t)core * IC_NUM_REGS);
    uint32_t cfg2 = *reg32(base, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET);
    cfg2 &= ~(
        (1 << LAGD_CORE_GLOBAL_CFG_2_CONFIG_VALID_AW_BIT) |
    ...
    *reg32(base, LAGD_CORE_GLOBAL_CFG_2_REG_OFFSET) = cfg2;
    ...
    cfg1 &= ~(
        (1 << LAGD_CORE_GLOBAL_CFG_1_DEBUG_DT_CONFIGURE_ENABLE_BIT) |
...
// Check and wait until computation under multi_cmpt_mode is done by polling MULTI_CMPT_MODE_IDLE bit in output_status register
...
// Compare the values in spin_fifo_data registers with the expected values and print the mismatch if any
static void lagd_check_spin_fifo_data(unsigned core) {
    ...
    if (pass0) {
        ...
        printf("[FAIL] expected %08x%08x%08x%08x%08x%08x%08x%08x\r\n",
               spin_ref_0[7], spin_ref_0[6], spin_ref_0[5], spin_ref_0[4],
               spin_ref_0[3], spin_ref_0[2], spin_ref_0[1], spin_ref_0[0]);
    }
    ...
    if (pass1) {
        ...
        printf("[FAIL] expected %08x%08x%08x%08x%08x%08x%08x%08x\r\n",
               spin_ref_1[7], spin_ref_1[6], spin_ref_1[5], spin_ref_1[4],
               spin_ref_1[3], spin_ref_1[2], spin_ref_1[1], spin_ref_1[0]);
    }
}
